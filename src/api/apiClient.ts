...
apiClient.interceptors.response.use(
  ...
  (error) => {
    ...
    if (error.code === 'ECONNABORTED') {
      ...
      error.message = 'Network error. Please check your connection and ensure MongoDB is running.';
    } else if (error.response.status === 500) {
      error.message = 'Server error. This might be related to the database connection.';
    }
